...
最快的方式，下载预构建数据库（1.8MB，含 537 名议员 + 完整画像 + 25963 个竞选委员会）：
...
预构建数据库包含：537 名现任议员基础信息、委员会任职、议员画像（对华立场、履历、教育背景）、25963 个竞选委员会。**前端可直接浏览议员列表、画像详情页和委员会概览，无需等待 FEC 献金数据导入。** 如需完整的 FEC 个人献金记录，后续运行：
